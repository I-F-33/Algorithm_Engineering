...
class BucketSortApplication:
    def __init__(self, root):
        ...
        self.upload_label = Label(self.frame, text="No file is uploaded")
        ...
        self.visual_button = Button(self.frame, text="Visualize Data", command=self.visualize_data, state='disabled')
        self.visual_button.pack(side='top', fill='x', pady=5)
    ...
    def sort_data(self):
        if self.data is not None:
            try:
                self.sorted_data = self.data.sort_values(by=['Sensor_ID', 'Temperature_C'], ascending=[True, True]).reset_index(
                    drop=True)
                self.show_table(self.sorted_data)
                self.visual_button.config(state='normal')
            except KeyError:
                messagebox.showerror("Error", "'Temperature_C' column not found in the file.")
    ...
    def visualize_data(self):
        if self.sort_data is not None:
            ...
            messagebox.showerror('Error', "No Sorted Data is available.")
